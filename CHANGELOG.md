...
### Export
- [fix]: Кнопка `#savePreview` в Telegram больше не молчит: PNG сразу кладётся в `previewState.blob` без `fetch(data:)`, `navigator.share({ files })` вызывается в жесте клика с таймаутом 1.6 с (на iOS/macOS Mini App шаринг пропускается — WebView зависает), при отказе копируется картинка в буфер или превью подсвечивается тостом «зажми картинку». Файлы: index.html, README.md.
