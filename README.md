...
11. Сохранение (`#savePreview`) не пишет PNG в `navigator.clipboard` (Android Telegram WebView даёт ложный успех, буфер пуст, `<a download>` режется). Цепочка: `tg.downloadFile({ url, file_name })` (Bot API 8.0+), затем `navigator.share({ files })` если `canShare({ files })` (на iOS/macOS Mini App шаринг пропускается — WebView зависает), иначе `window.open` / `tg.openLink` и подсказка «Зажми картинку для сохранения в галерею» на `#previewImg`.
