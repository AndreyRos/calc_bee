...
8. Шаринг (`shareResult`) открывает `t.me/share/url` с жёсткой ссылкой Mini App `https://t.me/calc_kalym_bot/app` (она же в тексте сообщения, чтобы клиент Telegram не подменял её на GitHub Pages); вызов через `tg.openTelegramLink` / `openLink`, фоллбэк — `window.open`.

## 2. Диагностический чеклист

- Telegram SDK: window.Telegram.WebApp инициализирован, first_name подставлен; шаринг идёт в `t.me/share/url` со ссылкой `https://t.me/calc_kalym_bot/app` (дублируется в тексте).
