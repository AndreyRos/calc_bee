...
6. Перед экспортом `runExport` вызывает `AdController.showRewarded()` (Adsgram rewarded). После успешного просмотра (или если SDK недоступен / `BLOCK_ID` ещё плейсхолдер) результат снимается через селект формата: Stories 9:16 PNG, сертификат PNG или гербовый PDF с номером № БАРАН-XXXX-VIP и шкалой 0–100. На каждом документе водяной знак со ссылкой на Mini App (`BOT_CONFIG` → t.me/calc_kalym_bot/app) и рекламный баннер «Место для вашей рекламы».
...
8. Шаринг (`shareResult`) открывает `t.me/share/url` на `https://andreyros.github.io/calc_bee/share.html` (Open Graph тянет `mainpic.jpg`, страница редиректит в Mini App). Текст: рейтинг Госкомитета, имя, бараны, ранг, CTA «А сколько барашек дадут за тебя? Нажми и узнай 👇» и ссылка `https://t.me/calc_kalym_bot/app`. Вызов через `tg.openTelegramLink` / `openLink`, фоллбэк — `window.open`.

## 2. Диагностический чеклист

- Telegram SDK: window.Telegram.WebApp инициализирован, first_name подставлен; шаринг идёт в `t.me/share/url` на `share.html` (превью `mainpic.jpg`) с ссылкой `https://t.me/calc_kalym_bot/app` в тексте.
...
Rewarded-ролик перед экспортом крутит Adsgram: SDK `https://sad.adsgram.ai/js/sad.min.js`, контроллер `AdController` в `index.html`. Подставь реальный `BLOCK_ID` из [partner.adsgram.ai](https://partner.adsgram.ai) вместо плейсхолдера `YOUR_ADSGRAM_BLOCK_ID`. Пока ID не задан или SDK недоступен, `showRewarded()` резолвит Promise и не блокирует генерацию Stories/сертификата. Пропуск или ошибка ролика реджектит Promise — экспорт не стартует. События `onStart` / `onReward` пишутся в `rewarded_shows` и `rewarded_completions`.
