...
9. Stories (`#storyShareBtn`, видна при формате `stories` внутри Telegram): `tg.shareToStory(media_url, params)` открывает нативный редактор. `shareToStory` принимает только публичный HTTPS URL (не `data:`), поэтому медиа — `https://andreyros.github.io/calc_bee/mainpic.jpg`, в caption — «Мой калым: N баранов 🐑», `widget_link` ведёт на `https://t.me/calc_kalym_bot/app`. Если метода нет — toast «Публикация Stories не поддерживается вашей версией Telegram».
10. Сертификат в чат бота (`#sendToBotBtn` на результате и `#sendToBotPreviewBtn` в модалке): `tg.sendData(JSON.stringify({ action: 'send_certificate', name, rams, verdict, serial, format }))`. Метод доступен только если Mini App открыт с keyboard-кнопки бота; после вызова клиент закрывает WebApp и отдаёт `web_app_data` боту. Иначе toast «Функция доступна только при запуске через кнопку бота в Telegram».
11. Сохранение (`#savePreview`) режется по платформе Telegram WebView: `data:` / blob нельзя отдать в `tg.downloadFile` (Android открывает мёртвое меню) и нельзя форсить `<a download>`. iOS/macOS — только long-press: подсветка `#previewImg` (`pointer-events`, `-webkit-touch-callout`, `user-select`) и toast «Зажмите картинку и выберите «Сохранить в Фото» 📲». Android — `navigator.share({ files })` при `canShare`, при отказе toast «Зажмите картинку и выберите «Сохранить изображение» 📥». Вне Telegram — `downloadBlob`.
...
- Экспорт: селект формата + `#exportBtn` вызывают html2canvas / jsPDF; на время генерации кнопка disabled и показывает спиннер 🐑 «Печатаем сертификат...», состояние возвращается в `finally`; `capture` снимает карточку с `x/y/scroll = 0` (не с `left: -12000px`), `useCORS: true` без `allowTaint`, `foreignObjectRendering: false`; макеты `#storiesCard` / `#certCard` рендерятся системными шрифтами, чтобы iOS не травил canvas Google Fonts; при SecurityError toast «Ошибка безопасности браузера. Переключаем на упрощенный рендер...» и `console.error('[export]', …)`; в превью подсказка «💡 Зажми картинку для сохранения в галерею»; `#savePreview` не зовёт `tg.downloadFile` с `data:`; на iOS только long-press («Сохранить в Фото»), на Android — `navigator.share({ files })` или long-press («Сохранить изображение»), в браузере — `downloadBlob`; `#storyShareBtn` видна только для формата `stories` в Telegram и зовёт `tg.shareToStory` с HTTPS `mainpic.jpg` + `widget_link`; `#sendToBotBtn` шлёт JSON через `tg.sendData` (keyboard-кнопка бота); ранг `CERT_TIERS` совпадает со шкалой; геометрический watermark ранга рисуется inline (CSS + Data-URI SVG) без CORS; штамп не перекрывает рекламу и водяной знак; ссылка бота кликабельна в PDF; ошибка не глотается — `console.error` + toast «Ошибка экспорта: …».
...
6. Для `sendData` (кнопка «Получить в чат бота») добавь боту Reply Keyboard с `KeyboardButton` типа `web_app` — метод работает только из такого запуска и закрывает Mini App, отдавая боту `Message.web_app_data`.
